...
export default function EventPageFixed() {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_IN' || event === 'SIGNED_OUT' || event === 'TOKEN_REFRESHED') {
        ...
        if (eventId) {
          setTimeout(() => checkParticipation(), 100); // Petit délai pour s'assurer que l'auth est bien mise à jour
        }
      }
    });
  ...
  }, [eventId, checkParticipation, supabase.auth]);
  ...
  // Effect pour re-vérifier la participation quand la page redevient visible
  useEffect(() => {
  ...
  }, [eventId, user, checkParticipation]);
