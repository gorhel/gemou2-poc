...
export default function EventPageWithParticipants() {
  ...
  const fetchParticipants = useCallback(async () => {
    ...
    try {
      ...
      // Trier : hôte en premier, puis participants par date d'arrivée
      participantsWithRoles.sort((a, b) => {
        if (a.role === 'host' && b.role !== 'host') return -1;
        if (b.role === 'host' && a.role !== 'host') return 1;
        return new Date(a.joined_at).getTime() - new Date(b.joined_at).getTime();
      });

      console.log('✅ Participants récupérés:', participantsWithRoles.length);
  ...
  }, [eventId, event?.creator_id, supabase]);
  ...
  // Effect pour charger les participants quand l'événement est chargé
  useEffect(() => {
    if (event) {
      fetchParticipants();
    }
              ...
              <CardContent>
                {creator ? (
                  <Link href={`/profile/${creator.username}`} className="block">
                    <div className="flex items-center space-x-4 hover:bg-gray-50 p-2 rounded-lg transition-colors">
          ...
          <Card className="mb-8">
            <CardHeader>
              <CardTitle className="text-xl flex items-center justify-between">
                ...
                {loadingParticipants && <LoadingSpinner size="sm" />}
              </CardTitle>
            </CardHeader>
            <CardContent>
              {participants.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">Soyez le premier à rejoindre cet événement !</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {participants.map((participant) => (
                    <Link 
                      ...
                      className="block"
                    >
                      <div className="flex items-center space-x-3 p-3 bg-white rounded-lg border border-gray-200 hover:border-gray-300 hover:shadow-sm transition-all">
