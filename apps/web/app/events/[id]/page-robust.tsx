...
export default function EventPageRobust() {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_IN' || event === 'SIGNED_OUT' || event === 'TOKEN_REFRESHED') {
        if (eventId) {
          setTimeout(() => checkParticipation(), 100);
        }
      }
    });
    ...
    try {
      ...
      if (isParticipating) {
        ...
        if (event) {
          ...
          setEvent(prev => prev ? { ...prev, current_participants: newCount } : null);
        }
        ...
        const { error: updateError } = await supabase
          .from('events')
          .update({ 
            current_participants: Math.max(0, event?.current_participants ? event.current_participants - 1 : 0)
          })
          .eq('id', eventId);
