...
export default function CreateEventForm({ onSuccess, onCancel }: CreateEventFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add selected tags to the event
      if (selectedTags.length > 0) {
        ...
        if (tagsError) {
          ...
          // Ne pas bloquer la création de l'événement si les tags échouent
        }
      }
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedTags.map((tagId) => {
                    // Note: Dans l'aperçu, on affiche juste les IDs. 
                    // En production, vous pourriez charger les noms des tags
                    return (
                      <span
                        key={tagId}
                        className="px-3 py-1 rounded-full text-sm font-medium bg-pink-100 text-pink-800 border-2 border-pink-300"
                      >
                        Tag {tagId.slice(0, 8)}...
                      </span>
                    );
